...
int set_cloexc(int); /* {Prog setfd} */
void clr_fl(int, int);
void set_fl(int, int); /* {Porg setfl} */
...
int serv_accept(int, uid_t *); /* {Prog seraccept_sockets} */
int cli_conn(const char *); /* {Prog cliconn_sockes} */
int buf_args(char *, int (*func)(int, char **)); /* {Prog bufargs} */

int tty_cbreak(int); /* {Porg raw} */
int tty_raw(int); /* {Prog raw} */
int tty_reset(int); /* {Prog raw} */
int tty_atexit(void); /* {Porg raw} */
struct termios *tty_termios(void); /* {Porg raw} */
...
pid_t lock_test(int, int, off_t, int, off_t); /* {Porg locktest} */

#define is_read_lockable(fd, offset, whence, len) (lock_test((fd), F_RDLCK, (offset), (whence), (len)) == 0)
#define is_write_lockable(fd, offset, whence, len) (lock_test((fd), F_WRLCK, (offsset), (whence), (len)) == 0)
